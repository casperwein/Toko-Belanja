...
const postProducts = async(req, res) => {
    ...
    await Category.findOne({ where: { id: CategoryId } }).then(category => {
        if (!category) {
            return res.status(400).json({ msg: `Category with id ${CategoryId} not found` })
        }
        Product.create({ title, price, stock, CategoryId }).then(product => {
            res.status(201).json({
                product: {
                    id: product.id,
                    title: product.title,
...
const updateCategoryId = async(req, res) => {
    ...
    await Category.findOne({ where: { id: CategoryId } }).then(result => {
        if (!result) {
...
const deleteProducts = async(req, res) => {
    const productId = req.params.productId;
    await Product.destroy({ where: { id: productId } }).then(() => {
